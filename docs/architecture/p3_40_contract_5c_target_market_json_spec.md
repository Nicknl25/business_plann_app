...
- R-d-bis → **SPLIT disposition** (was: deferred-as-single-item; surfaced by NexGen E2E run 2026-05-30 where the contract fired at `target_market_json_contract.py:256` against real production payload):
    - **CONTRACT-TYPING portion → RESOLVED** (commit pending — P3.41). `target_market_summary` retyped from `str` to `Optional[str] = None` at `target_market_json_contract.py` to mirror Contract 5d `key_people_summary` at `people_json_contract.py:294`. Production pops the field at intake_consult.py:10863 (commit e57ff49 single-source-of-truth pattern); contract now matches production reality per PSL2 production-reality-wins. Asymmetry resolved: Contract 5d had this disposition from the start; Contract 5c was missing it. Fixture `valid_target_market_json_dict` now defaults to OMITTED with `include_target_market_summary=True` opt-in toggle, mirroring 5d's pattern.
    - **GATE portion → still DEFERRED to intake-remediation workstream**. The `financials.py:95` + `intake_submit_service.py:224` gate-reads still need Option 3 (replace proxy-summary check with structural check on primary data). Currently bypassed via `_SKIP_INTAKE_REMEDIATION_GATES` flag introduced in P3.41 (commit 6a03377). MUST be set False before any production submission path is exercised. Same Option 3 fix applies symmetrically to `key_people_summary` (Contract 5d R-d gate portion).

  Original disposition text preserved for trace: Structurally identical to Contract 5d R-d (`key_people_summary`): (1) `target_market_summary` popped at intake_consult.py:10863 (intentional, same single-source-of-truth pattern as `key_people_summary`); (2) downstream gate-read at financials.py:95 hard-fails on the popped field; (3) the summary is a denormalized view of the per-segment target market data; (4) same Option 3 fix applies (replace proxy-summary check with structural check on primary data); (5) gate portion: intake-remediation workstream, not contract-layer cleanup. The original "single deferred item" framing missed that the contract-typing portion was independently fixable without touching intake; the NexGen E2E surfaced this and the contract-typing portion is now done.
