...
**Routine GPT review is forbidden.** GPT is engaged only when Python has
demonstrably failed a validator. Every existing routine-GPT-review site is
a target for conversion to this pattern (see handler inventory in §6).
...
|---|---|---|
| **Python (deterministic)** | Computation, defaults from cohort midpoints, mapping-formula evaluation, validator implementations, post-pass invariant checks. | Pretend to be deterministic while silently falling back on hardcoded universals. Use magic numbers to mask divergence. |
| **GPT (handler form ONLY)** | Judgment-heavy lever authoring inside a handler's defined authority. Trade-off reasoning when Python's default conflicts with intake-stated specifics. | Run routinely every plan. Author levers outside its defined authority. Operate without a validator-triggered entry point. |
...
State as of iter 19 build (incremental — each stage adds the next handler).
...
| **Funding** | `post_intake_funding_handler` | Debt issuance, debt repayment, owner's capital, other equity, distributions, `cash_strategy_mode` override | `cash_buffer_violations` non-empty after cash strategy post-pass | Built in Stage 4 |
| **Stage ramp** | `post_intake_stage_ramp_handler` | Stage ramp contract fields (ramp shape, target margins, capacity curve, cost ratio caps) | Stage ramp realism validator fails on deterministic Python ramp | Built in Stage 5 |
| **Payroll** | `post_intake_payroll_handler` | `expenses::Payroll` lever, headcount schedule grid, OEWS-catalog title selection, wage positioning multiplier | Payroll realism validators (payroll/revenue ratio, FTE sanity, target band compliance) fail | Built in Stage 6 |
| **Convergence** | `post_intake_gpt_exhaustion_handler` (reused) | 12 + 5 levers (same as restoration) | Restoration loop exhaustion in convergence flow | Made the only convergence-GPT entry in Stage 7 |
...
- **Routine GPT review.** Any GPT call that runs every plan regardless of
  validator state. Convert to Python-first + handler-on-failure.
