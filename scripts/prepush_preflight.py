...
def main() -> int:
    ...
    if any(_gate_relevant(f) for f in files):
        ...
        if code != 0:
            ...
            return 1
    # THE INTAKE PERSONA GATE IS GONE (Nick 2026-09-13). It cost $5 and days a
    # pass to replay conversations nobody read, four of seven passes existed
    # only because the app had rolled fresh wording a script had never seen,
    # and on the day it was retired all five legs were ERROR with 0 turns and
    # 0 GPT calls - the harness, not the app - while refusing a push carrying
    # a real client fix. Live evidence is a Cowork run; a fix is proven by
    # mini on the thing that changed and by the last five real drafts.
    # The known-issue gate above stays: it is the one that caught the seven.
    return 0
