...
def build_sources_and_uses(draft: Dict[str, Any]) -> Optional[Dict[str, Any]]:
  ...
  equity_in = Y("equity") + Y("other_equity") if _ysum(q, "equity", 1) is not None else 0.0
...
BODY_TABLE_BUILDERS = (build_condensed_statements, build_assumptions_table,
                       build_sources_and_uses, build_debt_amortization)
