...
# Legacy controller implementation kept for reference. The active strict controller is
# `python/client_intake_and_finmo/unified_intake/controller.py`.
...
from unified_intake.lobs import (
  ensure_lob_model_card as _ensure_lob_model_card,
  extract_lobs_from_text as _extract_lobs_from_text,
  slugify_lob_key as _slugify_lob_key,
)
...
from unified_intake.readiness import (
  financials_data_ready as _financials_data_ready,
  headcount_ready as _headcount_ready,
  marketing_ready as _marketing_ready,
  milestones_ready as _milestones_ready,
  model_has_required_drivers as _model_has_required_drivers,
  people_data_ready as _people_data_ready,
  revenue_ready as _revenue_ready,
  target_market_data_ready as _target_market_data_ready,
)
from unified_intake.redaction import strip_acs_codes as _strip_acs_codes
from unified_intake.model_engine import (  # type: ignore
  as_float_maybe as _as_float_maybe,
  ensure_pricing_from_ops as _ensure_pricing_model_card,
  get_company_total_lob as _get_company_total_lob,
  normalize_model_card_for_write as _normalize_model_card_for_write,
  recompute_headcount_company_total as _recompute_headcount_company_total,
  recompute_marketing_company_total as _recompute_marketing_company_total,
  recompute_revenue_company_total as _recompute_revenue_company_total,
  set_company_derived as _set_company_derived,
  set_company_driver as _set_company_driver,
)


def _client_message(*, kind: str, context: Dict[str, Any]) -> str:
  try:
    from unified_intake.language import render_client_message  # type: ignore

    return str(render_client_message(kind=str(kind), context=context or {}) or "").strip()
  except Exception:
    return ""
...
def post_intake_consult_handler(*, app, request):
  ...
  try:
    ...
    if interaction_mode not in ("chat", "button_only"):
      interaction_mode = "chat"
    focus_db = str(consult.get("active_focus") or "").strip() or "ops"

    # Canonical interaction rule:
    # - chat: free-text accepted and must produce a meaningful response.
    # - button_only: text input rejected; progress happens via model-card controls.
    if interaction_mode == "button_only":
      if not model_card_proposals:
        return (
          jsonify(
            {
              "error": "interaction_state_invalid",
              "detail": "button_only requires pending proposals",
              "draft_id": str(draft_id).strip(),
              "client_id": client_id,
              "active_focus": focus_db,
    ...
    # Always-on extraction + persistence: for every non-starting chat turn, capture any
    # canonical facts implied by the user's message and write them to SQL immediately,
    # before any card/rollup logic runs.
    ...
    if (not starting) and message:
      ...
      # Router-only messages should appear as a preface, not as a separate turn.
      ...
      if patch:
        ...
        prev_marketing_model_json = dict(marketing_model_json)
        prev_pricing_model_json = dict(pricing_model_json)
        prev_revenue_model_json = dict(revenue_model_json)
        prev_headcount_model_json = dict(headcount_model_json)
        prev_fulfillment_model_json = dict(fulfillment_model_json)
        prev_ops_concept_model_json = dict(ops_concept_model_json)
        prev_milestones_model_json = dict(milestones_model_json)
        prev_cogs_model_json = dict(cogs_model_json)
        prev_gna_model_json = dict(gna_model_json)

        # Split patch into canonical facts vs. model-card driver updates.
        ...
        for raw_key, val in (patch or {}).items():
          ...
          if group in ("business", "ops", "market", "people", "financials"):
            # Disambiguation: users sometimes describe a Year-1 staffing plan in plain language
            # ("3 staff at $20/hr ..."). If the intent router returns that as people.people,
            # treat it as a headcount.roles model update instead of corrupting People data.
            if group == "people" and str(_field or "").strip() == "people":
              try:
                maybe_roles = val
        ...
        try:
          for raw_key in (fact_patch or {}).keys():
            key = str(raw_key or "").strip()
            if key.count(".") != 1:
              continue
            group, _field = key.split(".", 1)
            group = group.strip().lower()
            if group and group not in touched_groups:
              touched_groups.append(group)
        except Exception:
          touched_groups = []

        # Track fact revisions as immutable history (facts are superseded, not blended).
        ...
        active_focus_out: str | None = None
        if str(draft_status).strip().lower() == "completed" or focus == "done":
          status_out = "in_progress"
          active_focus_out = next(
            (g for g in touched_groups if g in ("ops", "market", "people", "financials")),
            "ops",
          )

        # Apply model-card driver patches (stored in *_model_json columns) and compute derived rollups.
        model_changed = False
        ...
        try:
          for raw_key, value in (model_patch or {}).items():
            ...
            if group == "marketing":
              if field == "monthly_marketing_budget":
                unit, time_basis = "USD", "month"
              marketing_model_json, changed = _set_company_driver(
              ...
              if changed:
                model_changed = True
                driver_changes.append({"model": "marketing", "lob_key": "company_total", "path": f"drivers.{field}", "new": value})
            elif group == "revenue":
              if field in ("units_per_week_capacity", "avg_units_per_week_year1"):
                unit = str((ops_json or {}).get("unit_name") or "").strip() or "units"
                time_basis = "week"
              elif field == "operating_weeks_per_year":
                unit, time_basis = "weeks", "year"
              elif field == "unit_price":
                unit, time_basis = "USD", "per_unit"
              revenue_model_json, changed = _set_company_driver(
                revenue_model_json,
                key=field,
                value=value,
                unit=unit,
              ...
              if changed:
                model_changed = True
                driver_changes.append({"model": "revenue", "lob_key": "company_total", "path": f"drivers.{field}", "new": value})
            elif group == "pricing":
              if field == "unit_price":
                unit, time_basis = "USD", "per_unit"
              pricing_model_json, changed = _set_company_driver(
                pricing_model_json,
                key=field,
                value=value,
                unit=unit,
              ...
              # Convenience mirror: enables JSON_EXTRACT(pricing_model_json,'$.unit_price').
              if changed and field == "unit_price":
                try:
                  pricing_model_json["unit_price"] = float(value)
                except Exception:
                  pricing_model_json["unit_price"] = value
                # Keep canonical ops unit_price in lockstep so downstream revenue/COGS math
                # recomputes immediately in the same chat turn.
                try:
                  ops_json["unit_price"] = float(value)
                except Exception:
                  ops_json["unit_price"] = value
              if changed:
                model_changed = True
                driver_changes.append({"model": "pricing", "lob_key": "company_total", "path": f"drivers.{field}", "new": value})
            elif group == "headcount":
              headcount_model_json, changed = _set_company_driver(
              ...
              if changed:
                model_changed = True
                driver_changes.append({"model": "headcount", "lob_key": "company_total", "path": f"drivers.{field}", "new": value})
            elif group == "fulfillment":
              fulfillment_model_json, changed = _set_company_driver(
              ...
              if changed:
                model_changed = True
                driver_changes.append({"model": "fulfillment", "lob_key": "company_total", "path": f"drivers.{field}", "new": value})
            elif group == "ops_concept":
              ops_concept_model_json, changed = _set_company_driver(
              ...
              if changed:
                model_changed = True
                driver_changes.append({"model": "ops_concept", "lob_key": "company_total", "path": f"drivers.{field}", "new": value})
            elif group == "milestones":
              milestones_model_json, changed = _set_company_driver(
              ...
              if changed:
                model_changed = True
                driver_changes.append({"model": "gna", "lob_key": "company_total", "path": f"drivers.{field}", "new": value})
        except Exception:
          model_changed = model_changed

        # Deterministic rollups / derived recompute (company_total only).
        try:
          if marketing_model_json and marketing_model_json != prev_marketing_model_json:
            marketing_model_json, year1_marketing_spend_out = _recompute_marketing_company_total(
              marketing_model_json, now_ms=now_ms
            )
          if headcount_model_json and headcount_model_json != prev_headcount_model_json:
            headcount_model_json, year1_payroll_out = _recompute_headcount_company_total(
              headcount_model_json, now_ms=now_ms
            )
          if revenue_model_json or any(k.startswith("revenue.") for k in (model_patch or {}).keys()):
            revenue_model_json, ops_json, year1_revenue_out = _recompute_revenue_company_total(
              revenue_model_json, ops_json=ops_json, now_ms=now_ms
            )
          if cogs_model_json and (
            cogs_model_json != prev_cogs_model_json
            or revenue_model_json != prev_revenue_model_json
          ...
          if gna_model_json and gna_model_json != prev_gna_model_json:
            ...
            if recompute_gna_company_total:
              gna_model_json, year1_gna_total_out = recompute_gna_company_total(
                gna_model_json, now_ms=now_ms
              )
        except Exception:
          pass

        # Driver event log (lightweight audit).
        driver_nonce_out: int | None = None
        driver_events_out: List[Dict[str, Any]] | None = None
        if model_changed and driver_changes:
          try:
            current_driver_nonce = int(consult.get("driver_revision_nonce") or 0)
          except Exception:
            current_driver_nonce = 0
          try:
            existing_events = consult.get("driver_events_json")
            parsed_events = json.loads(str(existing_events)) if existing_events else []
            if not isinstance(parsed_events, list):
              parsed_events = []
          except Exception:
            parsed_events = []
          next_driver_nonce = current_driver_nonce + 1
          parsed_events.append(
            {
              "nonce": next_driver_nonce,
              "at_ms": now_ms,
              "action": "chat_patch",
          ...
          if len(parsed_events) > 500:
            parsed_events = parsed_events[-500:]
          driver_nonce_out = next_driver_nonce
          driver_events_out = parsed_events
        ...
        append_messages(
          ...
          business_facts=business_facts,
          operating_model_json=(ops_json if ("ops" in touched_groups or model_changed) else None),
          target_market_json=(market_json if "market" in touched_groups else None),
          people_json=(people_json if "people" in touched_groups else None),
          financials_json=(financials_json if "financials" in touched_groups else None),
          marketing_model_json=(marketing_model_json if marketing_model_json != prev_marketing_model_json else None),
          pricing_model_json=(pricing_model_json if pricing_model_json != prev_pricing_model_json else None),
          revenue_model_json=(revenue_model_json if revenue_model_json != prev_revenue_model_json else None),
          headcount_model_json=(headcount_model_json if headcount_model_json != prev_headcount_model_json else None),
          fulfillment_model_json=(fulfillment_model_json if fulfillment_model_json != prev_fulfillment_model_json else None),
          ops_concept_model_json=(ops_concept_model_json if ops_concept_model_json != prev_ops_concept_model_json else None),
          milestones_model_json=(milestones_model_json if milestones_model_json != prev_milestones_model_json else None),
          cogs_model_json=(cogs_model_json if cogs_model_json != prev_cogs_model_json else None),
          gna_model_json=(gna_model_json if gna_model_json != prev_gna_model_json else None),
        ...
        # Ensure downstream logic sees the latest values in this request.
        try:
          ...
          if isinstance(shared_context_live.get("model_cards"), dict):
            model_cards_live = dict(shared_context_live.get("model_cards") or {})
          else:
            model_cards_live = {}
          model_cards_live["pricing"] = pricing_model_json
          model_cards_live["revenue"] = revenue_model_json
          model_cards_live["marketing"] = marketing_model_json
          model_cards_live["headcount"] = headcount_model_json
          model_cards_live["fulfillment"] = fulfillment_model_json
          model_cards_live["ops_concept"] = ops_concept_model_json
          model_cards_live["milestones"] = milestones_model_json
          model_cards_live["cogs"] = cogs_model_json
          model_cards_live["gna"] = gna_model_json
          shared_context_live["model_cards"] = model_cards_live
        ...
        # Treat patches as part of normal chat flow; the domain consultant will respond.
        action = "continue_chat"
        router_msg = ""
        patch = None
    ...
    try:
      ...
      if lobs and not (isinstance(ops_concept_model_json.get("lobs"), list) and ops_concept_model_json.get("lobs")):
        ...
        cogs_model_json = _ensure_lob_model_card(cogs_model_json or {}, lobs)
        gna_model_json = _ensure_lob_model_card(gna_model_json or {}, lobs)
        append_messages(
          conn,
          draft_id=str(draft_id).strip(),
          new_messages=[],
    ...
    if starting:
      ...
      if focus == "done":
        assistant_text = _client_message(
          kind="complete",
          context={
            "draft_id": str(draft_id).strip(),
            "business": business_facts,
      ...
      if (
        ...
        if not any(isinstance(p, dict) and p.get("model") == "headcount" for p in model_card_proposals):
          proposal_id = f"hc_{now_ms}_{len(model_card_proposals)+1}"
          model_card_proposals = [
            *model_card_proposals,
            {
              ...
              "derived": [
                {
                  "key": "year1_payroll",
                  "value": consult.get("year1_payroll"),
                  "unit": "USD",
                  "time_basis": "year",
      ...
      if str(focus or "").strip().lower() == "ops":
        ...
        if ops_has_min_for_models and _revenue_ready(revenue_model_json) and not _milestones_ready(milestones_model_json):
          ...
          if not any(isinstance(p, dict) and p.get("model") == "milestones" for p in model_card_proposals):
            proposal_id = f"ms_{now_ms}_{len(model_card_proposals)+1}"
            model_card_proposals = [
              *model_card_proposals,
              {
      ...
      if focus == "ops":
                ...
                *model_card_proposals,
                {
                  ...
                  "derived": [
                    {
                      "key": "year1_marketing_spend",
                      "value": consult.get("year1_marketing_spend"),
                      "unit": "USD",
                      "time_basis": "year",
                      "derivation": "monthly_marketing_budget x 12",
                    }
        ...
        turn = {
          "assistant_message": _client_message(
            kind="continue",
            context={"draft_id": str(draft_id).strip(), "business": business_facts, "shared_context": shared_context},
          ),
    ...
    # People: once capability facts are ready, continue into headcount in chat mode.
    if (
      str(focus or "").strip().lower() == "people"
      and _people_data_ready(people_json=people_json)
      and not _headcount_ready(headcount_model_json)
    ...
    # Any extractable facts from this message have already been captured and persisted
    # above (before card/rollup logic). Continue with normal consult flow.
    ...
    if action == "edit_patch" and patch:
      ...
      if False:
        ...
        if followup_section_complete and followup_focus in ("ops", "market", "people", "financials"):
          ...
          ui_events_out: List[Dict[str, Any]] = []
          if str(next_focus_after_final or "").strip() and str(next_focus_after_final) != str(followup_focus):
            ui_events_out.append(
              {
                "type": "focus_changed",
                "from_focus": str(followup_focus),
                "to_focus": str(next_focus_after_final),
              }
            )
          ...
          if next_focus_after_final == "done":
            ...
            completion_msg = _client_message(
              kind="complete",
              context={
                "draft_id": str(draft_id).strip(),
                "business": business_facts,
    ...
    ui_events_out: List[Dict[str, Any]] = []
    if str(next_focus or "").strip() and str(next_focus) != str(focus):
      ui_events_out.append(
        {
          "type": "focus_changed",
          "from_focus": str(focus),
          "to_focus": str(next_focus),
        }
      )
    ...
    assistant_out = "\n\n".join([t for t in (assistant_final, next_text) if str(t or "").strip()]).strip()
    if preface:
