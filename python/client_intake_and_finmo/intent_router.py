  ...
  *,
  ...
  if isinstance((shared_context or {}).get("coherence_controller"), dict):
    extra_instructions = (
      ...
      "- If the client wants to pause, defer, come back later, stop for now, WRAP UP or FINISH the intake with the numbers as they stand, or says they are done with the levers (any phrasing that means they want to stop working the gap), return edit_patch with coherence.parked = true and nothing else. Never pressure them to continue, and never answer a stop with another lever.\n"
      "- A REFUSAL BINDS. If the client says a cost or a lever is off the table - the lease is signed, rent cannot move, leave the crews alone, no more volume, no more price changes, I do not want either of those lines - return coherence.assert_floor for it (rent, payroll, marketing, gna, cogs, or the round: pricing, volume, new_lines, cost_structure) EVEN IF the same message accepts something else. Never return an option id whose changes include a cost or lever the client just refused; if the message both refuses one thing and accepts another, emit the floor and, only if an offered option leaves the refused thing untouched, that option.\n"
