...
def _resolve_finmo_path(finmo_path=None):
    if finmo_path and str(finmo_path).strip():
...
def populate_finmo(business_type, finmo_path=None):
    conn = get_conn()
    ...
    try:
        ...
        # --- Load Excel ---
        resolved_path = _resolve_finmo_path(finmo_path)
        if not resolved_path:
            raise RuntimeError("FINMO path is not configured (set FINMO env or pass finmo_path).")
