...
- Fact-bearing template rule, and it applies ONLY to the STORED narrative fields (the paragraphs and summaries) - NEVER in assistant_message, the sentence the client reads on screen, because nothing renders a chat reply and the placeholder would reach her exactly as typed: if you mention the business name or upstream Ops facts in a STORED field, use placeholders like {{fact:business.name}}, {{fact:ops.product_pricing_summary}} (the whole pricing statement for multi-product businesses), {{fact:ops.unit_name}}, and {{fact:ops.unit_price}} (do not print literal values).
