...
1. Python defines the legal move space, structural constraints, issue packets, and required 20-quarter response shape.
...
- Full 20-quarter target coverage is required.
...
- `required_quarter_target_scaffold` is the response shape you should fill, quarter by quarter, for the full 20-quarter horizon.
...
- Meaningful progress is defined by Python as a net reduction in canonical `remaining_issue_count`. Flat issue count or a net increase is failure for the prior cycle.
...
- Quarter-specific primary targets for all 20 quarters
...
- `targets_by_quarter`: for every quarter 1 through 20, provide metric targets only for the chosen primary metrics
...
- Set targets quarter by quarter.
