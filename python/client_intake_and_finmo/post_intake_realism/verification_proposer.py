...
For each issue, the proposer maps:
  ...
  - some candidate levers WERE applied but not in every affected quarter
    OR the issue had no repair targets → "improved"
  - no candidate levers WERE applied → "stalled"

GPT then critiques specific issue verdicts, e.g. flipping "improved" to
"resolved" when the consultant judges the residual gap acceptable. When
...
def propose_realism_verification_payload(
  *,
  ...
  Statuses:
    ...
    - "improved": some applied updates landed; some affected quarters remain
    - "stalled": no candidate lever was touched
    - "needs_review": packet was malformed; cannot derive a verdict
  """
  ...
  per_status_counts: Dict[str, int] = {"resolved": 0, "improved": 0, "stalled": 0, "needs_review": 0}
  for issue in issue_packets_safe:
    verdict = _verdict_for_issue(issue=issue, applied_index=applied_index)
    severity = _clean(issue.get("severity")) or "medium"
    ...
    if status == "resolved":
      ...
      remaining_materiality = "none"
      observed_improvement_summary = (
        f"Convergence pass applied lever updates across all affected quarters; "
        f"issue gap is plausibly closed."
      )
    elif status == "improved":
      remaining_severity_score = max(1, int(round(pre_severity_score * 0.5)))
      remaining_materiality = "moderate"
      observed_improvement_summary = (
      ...
      remaining_severity_score = pre_severity_score or 50
      remaining_materiality = "high" if severity in ("high", "critical") else "moderate"
      observed_improvement_summary = (
        "No applied update touched any candidate lever; original gap stands."
      )
    else:
      remaining_severity_score = pre_severity_score or 25
      remaining_materiality = "needs_review"
      observed_improvement_summary = "Cannot derive verdict from applied_updates alone."
    issue_results.append(
      {
        "issue_code": _clean(issue.get("issue_code")),
        "status": status,
        "remaining_issue_materiality": remaining_materiality,
        "remaining_issue_severity_score": remaining_severity_score,
        "verification_reason": verdict["rationale"],
        "remaining_problem_quarters": list(verdict["remaining_problem_quarters"]),
  ...
  improved_count = per_status_counts.get("improved", 0)
  stalled_count = per_status_counts.get("stalled", 0)
  if total_count == 0:
    overall_assessment = "no_open_issues"
    executive_summary = "No issue packets were produced by the convergence pipeline; nothing to verify."
  elif stalled_count == 0 and improved_count == 0:
  ...
  elif resolved_count == 0 and improved_count == 0:
    overall_assessment = "no_progress"
    executive_summary = (
      f"No applied updates landed on any of the {total_count} issue(s); "
      "convergence pass made no measurable progress."
    )
  else:
    overall_assessment = "partial_resolution"
    executive_summary = (
      f"{resolved_count} of {total_count} issue(s) plausibly resolved; "
      f"{improved_count} improved; {stalled_count} stalled. Review residual quarters."
    )
