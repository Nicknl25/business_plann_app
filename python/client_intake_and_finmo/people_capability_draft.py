...
def ensure_table(conn) -> None:
  ...
  try:
    cur.execute(
      """
      CREATE TABLE IF NOT EXISTS intake_people_capability_drafts (
...
def create_draft(conn, *, draft_id: str, client_id: str) -> Dict[str, Any]:
  ...
  try:
    cur.execute(
      """
      INSERT INTO intake_people_capability_drafts
...
def get_draft(conn, *, draft_id: str) -> Dict[str, Any]:
  ...
  try:
    cur.execute(
      "SELECT * FROM intake_people_capability_drafts WHERE draft_id = %s LIMIT 1",
      (draft_id,),
    )
    row = cur.fetchone()
  finally:
  ...
  if not row or not isinstance(row, dict):
    raise RuntimeError(f"People & capability draft not found for draft_id={draft_id!r}")
  return row
...
def append_messages(
  ...
  *,
  ...
  if flat_fields:
    ...
    cols = _table_columns(conn, "intake_people_capability_drafts")
  ...
  sql = (
    "UPDATE intake_people_capability_drafts SET "
    + ", ".join(set_parts)
    + " WHERE draft_id = %s"
  )
...
def reopen_draft(conn, *, draft_id: str) -> None:
  ...
  try:
    cur.execute(
      """
      UPDATE intake_people_capability_drafts
