...
_PLANNING_MODE_DEFAULTS: Dict[str, str] = {
  "turnaround": (
    ...
    "Later-quarter baseline values (Q2 through Q20) are only placeholders and must NEVER be treated as authoritative targets or fixed constraints anywhere in the plan. "
    "Under NO circumstances treat baseline placeholder values in later quarters as fixed or directive. Do NOT preserve, lock, or allow later-quarter baseline values to dominate or unduly influence planning. "
    "Overriding these placeholders is mandatory whenever realism, strategy, or baby-AI cash constraints require changes. "
    "Enforce the quarter-by-quarter cash posture defined by baby AI strictly as binding constraints, not suggestions or soft guidelines. "
    "Make the business plan truly work quarter by quarter, aligning cash with baby AI intent without softening the cash law. "
    "Use the full company context and behave exactly like an operator trying to make the company truly work, not just cosmetically improve."
  ),
  "normalize": (
    "This case may be over-optimistic or commercially overstated rather than distressed. "
    "Later-quarter baseline figures are synthetic scaffolds and MUST NEVER be treated as authoritative constraints or fixed targets anywhere. "
    "Your job is to normalize the plan to something believable for the company's stage and business model. "
    "Dial back unrealistic pricing, utilization, capacity, margins, cash build, or timing as needed, and explicitly OVERRIDE any later-quarter baseline inputs that contradict intended cash posture or realism constraints. "
    "Business stage matters. Preserve real upside where truly believable, but REMOVE or ADJUST any soft baseline lock-in in favor of realism and the binding baby-AI cash posture."
  ),
  "rebalance": (
    "This case appears directionally sound but misbalanced. "
    "Later-quarter baseline inputs are NOT authoritative and should be overridden to maintain realism and baby-AI cash posture integrity EXACTLY. "
    ...
    "Never treat baseline as fixed; enforce baby-AI cash constraints strictly across all 20 quarters."
  ),
}
...
def _cash_strategy_context(financials_json: Dict[str, Any]) -> Dict[str, Any]:
  ...
  selected["advisory_only"] = False
  selected["constraint_rule"] = (
    "Treat the selected cash strategy as a real planning constraint that must be expressed quarter by quarter through believable row behavior. "
    "Do not treat it as optional flavor text."
  )
  ...
  selected["magnitude_rule"] = (
    "It is not enough to move rows in the right direction by token amounts. "
    "If the chosen strategy implies meaningful deployment, retention, or extraction, the affected rows must show believable magnitude for this business type."
  )
  return selected


def _row_semantics_for_prompt(row_id: str) -> str:
  normalized = str(row_id or "").strip()
  if not normalized:
    return ""
  lower = normalized.lower()
  if "::capacity" in lower:
    return "This is the physical or staffing throughput ceiling. Move it when the business gains or loses real capacity such as rooms, locations, providers, equipment, hours, or production capability."
  if "::utilization" in lower:
    return "This is the percentage of available capacity actually used. It should ramp with demand, execution, and maturity, not exceed believable operating intensity."
  if "::unit price" in lower or "::price" in lower:
    return "This is realized average pricing or mix. Use it for believable pricing power, discounting, or mix shift, not as a free lever to force cash."
  if lower == "expenses::cost of goods sold":
    return "This is the direct cost burden tied to revenue. Use it to reflect product/service mix, input costs, and operating realities, not as a generic cash plug."
  if lower == "expenses::marketing":
    return "This is demand-creation spend. It can step up around launches, expansion windows, new locations, or growth pushes, and can step down when the strategy is more conservative."
  if lower == "expenses::payroll":
    return "This is staffing and labor cost. Use it to reflect hiring, provider growth, admin buildout, or operating support needs. Payroll should move when scale, locations, hours, or service complexity move."
  if lower == "expenses::general & administrative":
    return "This is recurring overhead. It should scale with business complexity and infrastructure, but it is not the main place to hide large cash movements."
  if lower == "expenses::lease/rent":
    return "This reflects facility footprint and leased operating space. Step it when the business adds or upgrades locations, rooms, or major space commitments."
  if lower == "expenses::research & development":
    return "Use this only if the business truly spends on development, product creation, or meaningful innovation work. Do not force it into businesses where it does not belong."
  if lower == "expenses::taxes":
    return "Taxes should follow profitability and tax posture. Do not use taxes as an arbitrary balancing row."
  if lower == "balance_sheet::accounts receivable days":
    return "Higher AR days means customers pay later and cash is tied up longer. Lower AR days means faster collections and better cash conversion."
  if lower == "balance_sheet::inventory days":
    return "Higher inventory days ties up more cash in stock. Use it when the business needs more working inventory, product breadth, or safety stock."
  if lower == "balance_sheet::accounts payable days":
    return "Higher AP days means vendors are paid later and cash is preserved longer. Lower AP days means faster payment and more cash use."
  if lower == "balance_sheet::prepaid expenses (% of revenue)":
    return "Higher prepaids use cash earlier for rent, insurance, subscriptions, or similar items. Keep it small unless the business model really prepays meaningful costs."
  if lower == "balance_sheet::deferred revenue (% of revenue)":
    return "Higher deferred revenue brings cash in before revenue is recognized, such as packages, deposits, subscriptions, or prepayments. Use it only when the business model supports advance cash collection."
  if lower == "balance_sheet::owner's capital":
    return "This row represents owner equity invested in or withdrawn from the business through changes over time. Step changes imply injections or distributions, not routine operating expense."
  if lower == "balance_sheet::other equity":
    return "This row represents non-owner equity movements or other equity financing changes. Only move it when the capital structure story supports it."
  if lower == "schedules::plus: additions (repayments), net":
    return "This is net new borrowing or debt reduction. Positive values bring in financing cash and raise debt; negative values use cash to repay debt. It is a financing lever, not an operating plug."
  if lower == "schedules::capital expenditures":
    return "This is discretionary investment in equipment, buildout, facilities, locations, rooms, major systems, or other long-lived assets. It is not just a routine bill. Use it when the business type supports real reinvestment or capacity expansion, and make the magnitude large enough to match any claimed expansion story."
  if lower == "schedules::less: principal repayments":
    return "This is contractual or chosen cash paydown of financed asset or lease principal. Higher values use more cash to delever; lower values preserve cash."
  if lower == "schedules::plus: net additions":
    return "This is new financed-asset or lease additions. Use it when the business acquires meaningful leased or financed long-lived assets rather than paying cash upfront."
  if lower == "cash":
    return "This is the resulting ending-cash posture implied by the other rows. Do not treat it like an isolated row; make the operating and financing rows produce the intended cash behavior."
  if lower == "revenue":
    return "This is the resulting top-line output implied by price, capacity, utilization, and economics. Keep it believable for the business type and stage."
  if lower == "ebitda":
    return "This is the resulting operating profitability implied by the full row set. It should be believable, not forced."
  return ""


def _row_semantics_prompt_block(grid_rows: List[Dict[str, Any]]) -> str:
  lines: List[str] = []
  seen: set[str] = set()
  for item in grid_rows:
    ...
    if not row_id or row_id in seen:
      continue
    seen.add(row_id)
    guidance = _row_semantics_for_prompt(row_id)
    if guidance:
      lines.append(f"- {row_id}: {guidance}")
  if not lines:
    return ""
  return "Row semantics and how these rows express strategy:\n" + "\n".join(lines) + "\n\n"
...
def build_quarter_grid_prompt(
  *,
  ...
  row_semantics_block = _row_semantics_prompt_block(grid_rows)
  return "".join(
    [
      ...
      "- cash strategy is not advisory flavor text; it is a real planning requirement that should be visible quarter by quarter through the row behavior you choose\n",
      "- do not override core business drivers like revenue realism, operating capacity, or baseline feasibility\n",
      ...
      "- use the rows as real economic levers: if the business type supports expansion, capacity buildout, new facilities, new locations, equipment, financing changes, staffing growth, or working-capital deployment, you are allowed to express that through the appropriate existing rows\n",
      "- do not be timid with credible deployment when the business type and selected strategy support it; excess cash does not need to remain trapped on the balance sheet if there is a believable use for it\n",
      "- infer the believable deployment or retention channels from the business type itself; do not force the same mechanism across all businesses, but do make the chosen mechanism materially visible when the strategy calls for it\n",
      "- all four cash strategies must be materially visible in different ways: reinvest, preserve cash, shareholder return, and balanced should not collapse into one generic cash staircase with slightly different wording\n",
      "- choose the deployment mechanism that fits the business type: asset-light businesses may express growth more through hiring, marketing, working capital, lease/rent step-ups, or debt changes, while asset-heavy businesses may use capex and financed additions more heavily\n",
      "- make sure capex timing, hiring pace, marketing intensity, debt behavior, equity behavior, working-capital posture, and retained liquidity read like one coherent management posture\n",
      "- all rows must work together logically where the business mechanics interact; do not treat them as isolated knobs\n",
      ...
      "- for reinvest, look for believable future deployment opportunities the business type can actually absorb, such as capacity additions, equipment, facilities, provider growth, store/location expansion, systems, or marketing pushes, and express those through the existing rows rather than by hoarding cash\n",
      "- for preserve cash, keep a visibly more conservative liquidity posture and avoid overly eager cash deployment\n",
      "- for preserve cash, deploy only where realism truly requires it and let the business retain a stronger cushion instead of forcing unnecessary expansion\n",
      "- for shareholder return, avoid letting excess cash pile up indefinitely without a believable reason to retain it\n",
      "- for shareholder return, use the existing financing and equity rows where appropriate so the business does not look like it is hoarding cash without purpose\n",
      "- for balanced, show a middle path with some retained cushion and some selective deployment\n",
      "- for balanced, mix buildup and deployment in a measured way rather than collapsing into either pure hoarding or pure reinvestment\n",
      "- if the narrative implies a major action such as expansion, material redeployment, meaningful retention, or capital extraction, the rows must numerically support that claim with believable magnitude rather than tiny maintenance-scale changes\n",
      "- do not describe a major location, facility, or scaling story if the actual rows only show small maintenance-level numbers that do not support that story\n",
      ...
      "Q1 is the most anchored operating reference. Q2 through Q20 baseline values may be spread placeholders rather than true future intent, so do not preserve later-quarter baseline numbers by inertia when realism, strategy, and cash posture require override.\n",
...
def quarter_grid_system_prompt(*, use_real_strategy_prompt: bool, planning_mode: str, realism_memo_present: bool = False) -> str:
  ...
  return (
    ...
    "Treat Q1 as the most anchored operating reference quarter. Do not treat Q2 through Q20 baseline values as automatic future truth when the business logic, strategy, and cash posture support a different path.\n"
    "Use the existing rows as real economic levers and make them work together coherently rather than treating them as disconnected boxes.\n"
    + realism_boundary
    + planning_mode_text(planning_mode)
    + "\n"
