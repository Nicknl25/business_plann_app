...
def fill_break_even_formulas(ws, data: DraftWorkbookData, ctx: WorkbookBuildContext) -> None:
  ...
  for label, r in ctx.finmo_rows.get(BREAK_EVEN_STATEMENT, {}).items():
    ...
    ws.cell(row=r, column=2).font = Font(size=9, color="666666")
...
def write_cvp_helper_and_chart(ws, ctx: WorkbookBuildContext, *, start_row: int, anchor: Optional[str] = None, quarter_col: int = FIRST_LIVE_COL, title_suffix: str = "Q1") -> int:
  ...
  for c, name in ((col_x, "Revenue (x)"), (col_rev, "Total Revenue"), (col_cost, "Total Cost"), (col_be, "Break-Even Point"), (col_plan, "Planned Revenue"), (col_lbl, "Region")):
    ws.cell(row=hdr, column=c, value=name).font = Font(bold=True)
  ...
  xmax_row = hdr + 1
  ws.cell(row=xmax_row, column=col_x, value="X max").font = Font(italic=True, size=9)
  ...
  ws.cell(row=be_r, column=col_lbl, value="Break-even").font = Font(size=9)
  ...
  ws.cell(row=plan_r1, column=col_lbl, value="Planned revenue").font = Font(size=9)
  ...
  ws.cell(row=loss_r, column=col_lbl, value="LOSS").font = Font(size=9, bold=True, color="C00000")
  _set(ws, profit_r, col_x, f"=({be}+{xmax})/2")
  _set(ws, profit_r, col_be, f"=({be}+{xmax})/2")
  ws.cell(row=profit_r, column=col_lbl, value="PROFIT").font = Font(size=9, bold=True, color="008000")
  ...
  ws.add_chart(chart, anchor or f"AD{ctx.finmo_row(BREAK_EVEN_STATEMENT, 'Fixed Costs')}")
  return profit_r + 2
...
def build_cvp_chart(ws, *, first: int, last: int, be_r: int, plan_r1: int, plan_r2: int, loss_r: int, profit_r: int, title: str) -> ScatterChart:
  ...
  # openpyxl 3.1 charts omit axis labels unless delete is explicitly False.
  ...
  # Total Revenue (45-degree line)
  s_rev = Series(Reference(ws, min_col=2, min_row=first - 0, max_row=last), x, title="Total Revenue")
  s_rev.marker = Marker(symbol="none")
  s_rev.graphicalProperties.line.solidFill = "1F4E79"
  s_rev.graphicalProperties.line.width = 28575
  # Total Cost (fixed intercept + variable slope)
  s_cost = Series(Reference(ws, min_col=3, min_row=first, max_row=last), x, title="Total Cost (fixed + variable)")
  s_cost.marker = Marker(symbol="none")
  s_cost.graphicalProperties.line.solidFill = "C00000"
  s_cost.graphicalProperties.line.width = 28575
  # Break-even point (single marker with the $ value shown)
  s_be = Series(Reference(ws, min_col=4, min_row=be_r, max_row=be_r), Reference(ws, min_col=1, min_row=be_r, max_row=be_r), title="Break-Even Point")
  s_be.marker = Marker(symbol="diamond", size=12)
  s_be.marker.graphicalProperties.solidFill = "FFC000"
  s_be.graphicalProperties.line.noFill = True
  s_be.dLbls = DataLabelList()
  s_be.dLbls.showVal = True
  s_be.dLbls.showCatName = False
  s_be.dLbls.showSerName = True
  s_be.dLbls.showLegendKey = False
  s_be.dLbls.position = "r"
  # Planned revenue vertical marker
  s_plan = Series(Reference(ws, min_col=5, min_row=plan_r1, max_row=plan_r2), Reference(ws, min_col=1, min_row=plan_r1, max_row=plan_r2), title="Planned Revenue")
  s_plan.marker = Marker(symbol="none")
  s_plan.graphicalProperties.line.solidFill = "70AD47"
  s_plan.graphicalProperties.line.dashStyle = "dash"
  s_plan.graphicalProperties.line.width = 19050
  # LOSS / PROFIT region labels
  s_loss = Series(Reference(ws, min_col=4, min_row=loss_r, max_row=loss_r), Reference(ws, min_col=1, min_row=loss_r, max_row=loss_r), title="LOSS region")
  s_loss.marker = Marker(symbol="triangle", size=9)
  s_loss.marker.graphicalProperties.solidFill = "C00000"
  s_loss.graphicalProperties.line.noFill = True
  s_loss.dLbls = DataLabelList(); s_loss.dLbls.showSerName = True; s_loss.dLbls.showVal = False; s_loss.dLbls.showCatName = False; s_loss.dLbls.showLegendKey = False; s_loss.dLbls.position = "t"
  s_profit = Series(Reference(ws, min_col=4, min_row=profit_r, max_row=profit_r), Reference(ws, min_col=1, min_row=profit_r, max_row=profit_r), title="PROFIT region")
  s_profit.marker = Marker(symbol="triangle", size=9)
  s_profit.marker.graphicalProperties.solidFill = "008000"
  s_profit.graphicalProperties.line.noFill = True
  s_profit.dLbls = DataLabelList(); s_profit.dLbls.showSerName = True; s_profit.dLbls.showVal = False; s_profit.dLbls.showCatName = False; s_profit.dLbls.showLegendKey = False; s_profit.dLbls.position = "b"
  for s in (s_rev, s_cost, s_be, s_plan, s_loss, s_profit):
